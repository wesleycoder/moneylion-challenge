...
const timelineDotVariants = cva(
  [
    'relative col-start-2 col-end-3 row-start-1 row-end-1 flex size-4 items-center justify-center',
    'rounded-full border border-current overflow-hidden',
  ].join(' '),
  {
    variants: {
      status: {
...
const TimelineDot = React.forwardRef<HTMLDivElement, TimelineDotProps>(
  ({ className, status, children, ...props }, ref) => (
    <div
      ...
      {...props}
    >
      {children ?? <div className="radix-circle size-2.5 rounded-full" />}
    </div>
  ),
)
...
const TimelineLine = React.forwardRef<HTMLHRElement, TimelineLineProps>(
  ({ className, done = false, ...props }, ref) => {
    return (
      <hr
        // biome-ignore lint/a11y/useAriaPropsForRole: TODO - replace with appropriate role
        role="separator"
